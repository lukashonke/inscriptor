<template>
  <q-card class="q-mb-md" :class="{ 'bg-red-1': !enabled }" bordered flat>
    <q-card-section class="q-px-md q-py-sm">
      ...
      <q-slide-transition>
        <q-card v-if="settingsExpanded" flat class="q-mt-sm settings-panel">
          <q-tabs
            v-model="selectedTab"
            class="clean-tabs"
          ...
          >
            ...
            <q-tab name="ai-settings" label="AI Settings & Parameters" />
          ...
          <q-tab-panels v-model="selectedTab" animated>
            <q-tab-panel name="general">
              <q-card-section class="q-gutter-y-md q-py-sm q-px-sm">
                <div class="row q-gutter-x-md items-center">
                  ...
                  <div class="col">
                    <q-input outlined filled dense label="Prompt Name" v-model="title" />
                  </div>
                  <div class="col-2" v-if="allowPromptCategorization">
                ...
                <div class="row">
                  <div class="col-12 q-mb-sm">
                    <InputWithAi v-model="description" label="Description" :prompt-ids="promptStore.getPredefinedPromptId('Prompt Description Generator')" :prompt-input="promptInput" />
                  </div>
                  <div class="col-12">
                    <q-input autogrow outlined filled dense label="Usage Guide / Tips" v-model="guide" />
                  </div>
                </div>
              </q-card-section>
            </q-tab-panel>

            <q-tab-panel name="ai-settings">
            ...
            <q-tab-panel name="advanced">
              <q-card-section class="q-pt-none q-mt-sm q-px-sm" v-if="showAdvancedSettings">
                ...
                <q-expansion-item label="Chat AI Agent Configuration" dense icon="mdi-robot-outline" v-if="showAdvancedSettings && showAgentChatSettings">
                  <q-card>
                    ...
                    <q-card-section class="q-gutter-y-md">
                      <div class="row">
                        <div class="col-auto">
                          <q-checkbox v-model="canBeUsedByAgent" dense label="Can be used by chat AI agent" />
                        </div>
                        <div class="col-auto flex items-center">
                          <HelpIcon tooltip="Allow external prompt agents to discover and use this prompt automatically"></HelpIcon>
                        </div>
                      </div>
                      ...
                      <div v-if="canBeUsedByAgent" class="row q-gutter-x-sm">
                        ...
                        <div class="col-auto items-center flex">
                          <HelpIcon tooltip="Pre-configured contexts will be automatically included when prompt agents execute this prompt, ensuring they have the right information to work effectively."></HelpIcon>
                        </div>
                      </div>
                    </q-card-section>
                  </q-card>
                ...
                <q-expansion-item :label="`Prompt Agents (${prompt.agents?.length ?? 0})`" dense v-if="allowAgents" icon="mdi-robot">
                  <q-card>
                    <q-card-section class="q-pb-none">
                      <div class="text-caption text-grey">Tip: Define prompt agents to further improve the output of this prompt before presenting it to you.</div>
...
const agentDefaultContextTypes = computed({
  get: () => props.prompt.agentDefaultContextTypes ?? [],
  set: (value) => {
    promptStore.updatePrompt(props.prompt, {agentDefaultContextTypes: value});
  }
});
